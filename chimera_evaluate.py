"""Evaluate AntigenForce predictions with full CHIMERA-Bench metric suite.
...
def load_numbering_scheme():
    ...
    return cfg.get("numbering_scheme", "imgt")
...
def main():
    parser = argparse.ArgumentParser(
        description=f"Evaluate {BASELINE} predictions with full CHIMERA metric suite",
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--predictions", type=Path,
                       help="Single predictions directory (.pt files)")
    group.add_argument("--aggregate", action="store_true",
                       help=f"Aggregate all CDR runs from results_root/{BASELINE}/")
    ...
    if args.predictions:
        ...
        per_complex, summary, cdr_type = run_full_evaluation(
            args.predictions, args.split, data_root=shared["paths"]["data_root"],
        ...
        results_root = Path(shared["paths"]["results_root"]) / BASELINE
    ...
    print(f"\nCHIMERA Full Evaluation Results ({BASELINE})")
    print(f"{'CDR':<6}", end="")
    for k in FULL_METRIC_KEYS:
        print(f" {k:>18}", end="")
    print()
